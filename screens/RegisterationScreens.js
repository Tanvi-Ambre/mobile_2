...
const RegistrationScreen = ({ navigation }) => {
 ...
 //console.log("db",db);
  ...
  const [permission, askForPermission] = useCameraPermission();
  ...
  useEffect(() => {
    askForPermission(); // Ask for camera permission when component mounts
  }, []);
...
const handleRegister = async () => {
  try {
    ...
    const { email, fullName, dateOfBirth, constituency, uvc } = voterData;

    const password = CryptoJS.SHA256(voterData.password).toString()
    ...
    await AsyncStorage.setItem('lastVoterId', email);
     ...
     <ScrollView>
      ...
      <View style={styles.uvcContainer}>
        <TextInput
          ...
          value={voterData.uvc}
          onChangeText={(value) => {
            handleChange('uvc', value);
            setScanned(false); // Clear scanned data when user manually enters UVC
          }}        />
