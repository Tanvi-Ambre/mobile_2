...
const ElectionDashboard = ({ navigation }) => {
    ...
    const loadResultsAnnounced = async () => {
      try {
        ...
        if (value !== null) {
          ...
          // If not previously announced, set it to false
    ...
    const loadAnnounceOutcome = async () => {
      try {
        ...
        if (value !== null) {
          ...
          // If not previously set, leave it as an empty string
        }
      } catch (error) {
  ...
  const saveResultsAnnouncedToStorage = async (value) => {
    try {
      await AsyncStorage.setItem('resultsAnnounced', JSON.stringify(value));
    } catch (error) {
      console.error('Error saving resultsAnnounced:', error);
    }
  };

  const saveAnnounceOutcomeToStorage = async (value) => {
    try {
      await AsyncStorage.setItem('announceOutcome', value);
    } catch (error) {
 ...
 */
  ...
  const announceResults = async() => {
    try {
      // Update the state with the election outcome
      let outcomeMessage = '';
  
      
       // console.log("resultsAnnounced", resultsAnnounced)
        outcomeMessage = 'You have announced the election outcome';
      ...
      //saveResultsAnnouncedToStorage(true);
      //saveAnnounceOutcomeToStorage(outcomeMessage);
